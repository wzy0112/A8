...
def build_sensor_order(df, direction):
    """
    Build manually defined detector order.

    NW_to_SE:
        From northwest to southeast on A8.
        Displayed from top to bottom.

    SE_to_NW:
        From southeast to northwest on A8.
        Displayed from bottom to top in your reference,
        so here we list it in top-to-bottom display order.
    """
    ...
    se_to_nw_order = [
        ...
        "MQ_8_11270_Stg",
        "MQ_8_11700_Stg",
        "LVE_77339003_33_34_35",
        "LVE_76339001_33_34_35",
    ]

    if direction == "NW_to_SE":
        manual_order = nw_to_se_order
    elif direction == "SE_to_NW":
        manual_order = se_to_nw_order
    else:
        raise ValueError("direction must be 'NW_to_SE' or 'SE_to_NW'")

    existing_sensors = set(df["sensor_id"].dropna().unique())

    sensor_order = [s for s in manual_order if s in existing_sensors]

    missing_sensors = [s for s in manual_order if s not in existing_sensors]
    if missing_sensors:
        print("[WARNING] These ordered sensors were not found in detector output:")
...
def visualize_baseline_heatmaps():
    ...
    # 3. Sensor ordering
    # =====================================
    sensor_order = build_sensor_order(df,direction="NW_to_SE")
