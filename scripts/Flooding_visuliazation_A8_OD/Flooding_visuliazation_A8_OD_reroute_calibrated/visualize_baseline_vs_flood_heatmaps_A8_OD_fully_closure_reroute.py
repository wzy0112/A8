...
def plot_difference_map(edge_geom, diff_df, value_col, title, colorbar_label, output_file):
    ...
    # 差值图统一规则：小 = 绿，中 = 黄，大 = 红
    # 只有 density 图 → 用 percentile 压缩色标 这个应该跟我之前的哪个结尾异常堵车有关 如果未来能够解决这个问题 可以统一出图
    # 其他图（speed / flow / vc） → 不变
    cmap = "RdYlGn_r"
    # vmin = 0.0
    ...
    if value_col == "density_increase":
        ...
        if positive_values:
            vmin = 0.0
            vmax = np.percentile(positive_values, 50)  # 🔥 关键：增强对比
        else:
            vmin = 0.0
            vmax = 1.0
...
def main():
    ...
    baseline_edge_file = os.path.join(project_root, "heat_edges", "a8_edge_output_od_reroute_calibrated.xml")
    flood_edge_file = os.path.join(project_root, "heat_edges", "a8_full_close_edge_output_od_reroute.xml")

    results_dir = os.path.join(project_root, "heat_edges")
    os.makedirs(results_dir, exist_ok=True)

    output_speed = os.path.join(results_dir, "diff_speed_drop_map_od_full_close_reroute.png")
    output_flow = os.path.join(results_dir, "diff_flow_change_map_od_full_close_reroute.png")
    output_density = os.path.join(results_dir, "diff_density_increase_map_od_full_close_reroute.png")
    output_vc = os.path.join(results_dir, "diff_vc_increase_map_od_full_close_reroute.png")
    output_csv = os.path.join(results_dir, "baseline_vs_flood_diff_od_full_close_reroute.csv")
