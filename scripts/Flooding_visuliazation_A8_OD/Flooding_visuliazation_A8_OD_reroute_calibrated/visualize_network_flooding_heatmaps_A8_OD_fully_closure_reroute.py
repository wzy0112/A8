...
def visualize_network_heatmaps():
    ...
    edge_output_file = os.path.join(project_root, "heat_edges", "a8_full_close_edge_output_od_reroute.xml")

    results_dir = os.path.join(project_root, "heat_edges")
    os.makedirs(results_dir, exist_ok=True)

    output_speed = os.path.join(results_dir, "flood_network_speed_heatmap_od_full_close_reroute.png")
    output_flow = os.path.join(results_dir, "flood_network_flow_heatmap_od_full_close_reroute.png")
    output_density = os.path.join(results_dir, "flood_network_density_heatmap_od_full_close_reroute.png")
    output_vc = os.path.join(results_dir, "flood_network_vc_heatmap_od_full_close_reroute.png")
    ...
    agg_df = aggregate_metrics(df, net)

    speed_df = agg_df[["edge_id", "speed"]].rename(columns={"speed": "value"})
    density_df = agg_df[["edge_id", "density"]].rename(columns={"density": "value"})
    flow_df = agg_df[["edge_id", "flow"]].rename(columns={"flow": "value"})
    vc_df = agg_df[["edge_id", "vc"]].rename(columns={"vc": "value"})
