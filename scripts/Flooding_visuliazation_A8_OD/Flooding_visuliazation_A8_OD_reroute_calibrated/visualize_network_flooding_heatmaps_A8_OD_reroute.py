...
def aggregate_edge_metric(df, metric_name):
    """
    Aggregate one edge metric by edge_id.
    Output columns must be: edge_id, value
    """
    if df.empty:
        return pd.DataFrame()
    ...
    if metric_name not in work_df.columns:
        print(f"[ERROR] Metric not found: {metric_name}")
        return pd.DataFrame()

    work_df.loc[work_df[metric_name] < 0, metric_name] = np.nan

    grouped = (
        work_df.groupby("edge_id", as_index=False)[metric_name]
        .mean()
...
def visualize_network_heatmaps():
    ...
    edge_output_file = os.path.join(project_root, "heat_edges", "a8_flood_edge_output_od_reroute_nottest.xml")

    results_dir = os.path.join(project_root, "heat_edges")
    os.makedirs(results_dir, exist_ok=True)

    output_speed = os.path.join(results_dir, "flood_network_speed_heatmap_od_reroute.png")
    output_flow = os.path.join(results_dir, "flood_network_flow_heatmap_od_reroute.png")
    output_density = os.path.join(results_dir, "flood_network_density_heatmap_od_reroute.png")
    output_vc = os.path.join(results_dir, "flood_network_vc_heatmap_od_reroute.png")
